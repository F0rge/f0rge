...
import { tenantBaseDomain, workspaceLoginUrl, slugFromHostInput } from "@/lib/site";
import { checkSlug } from "@/lib/slug";

export function SigninPicker() {
  const id = useId();
  const [value, setValue] = useState("");
  const slug = slugFromHostInput(value);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        e.preventDefault();
        if (ok) window.location.assign(workspaceLoginUrl(slug));
      }}
    >
