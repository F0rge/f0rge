...
- **Canvas (N7):** `chart_dining_vs_sofas` emits `canvas_spec` (allowlisted `bar` | `line` | `table` | `metric` — never HTML) with `path: /canvas`. Frontend `/canvas` (SideNav for `nia.use`) renders the spec; latest in `sessionStorage` key `vellano-nia-canvas-spec`. Tests: `tests/test_nia_canvas.py`.
