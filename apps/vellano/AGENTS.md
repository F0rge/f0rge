...
- **Threads (N1):** `GET/POST /api/v1/nia/threads`, `GET /api/v1/nia/threads/{id}`, `POST /api/v1/nia/threads/{id}/archive` — require `nia.use`. List returns current user's non-archived threads only; cross-user GET is 404.
...
- **Dock (N6):** Right-side Carbon panel in `app-shell.tsx` for users with `nia.use` (`canUseNia`). **Cmd/Ctrl+J** or header Nia mark toggles open/closed. Drag left edge to resize (sessionStorage); Expand/Dock for ~92% width. Microphone control uses Carbon `Microphone` icon with `aria-label="Dictate"` (Web Speech API; append transcript, no auto-send). Product name **Nia** only — never Copilot. `POST …/run` and accept/decline `…/resume` are AG-UI SSE — **consume the full stream** then `GET` thread; cancel resume is JSON. Usage meter in dock header from `GET /nia/usage/me` (hide on 404). Structured cards for `needs_ok` / `your_call` / `opened_page` / `overdue_invoices` / `canvas_spec` / `transfer_draft` — never raw JSON in chat.
