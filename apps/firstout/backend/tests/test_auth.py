"""HTTP-level auth tests for Firstout S1."""
...
from tests.conftest import OWNER_EMAIL, OWNER_PASSWORD, assert_firstout_session_cookie
...
async def test_login_returns_200_and_sets_cookie(async_client: AsyncClient) -> None:
    ...
    assert_firstout_session_cookie(resp)
...
async def test_me_authenticated_returns_200_with_team(async_client: AsyncClient) -> None:
    ...
    assert body["team"]["name"] == "Firstout"
    assert body["team"]["id"]
...
def test_no_ht_session_string_in_implementation() -> None:
    """Firstout must not reuse Marrow's ht_session cookie name."""
