"""Regression: migration 003 creates locations table and seed rows."""
...
async def test_migration_003_creates_locations_and_seed_rows(
    ...
    async with engine.connect() as conn:
        count = (await conn.execute(sa.text("SELECT COUNT(*) FROM locations"))).scalar_one()
        rows = (
            await conn.execute(sa.text("SELECT name, type FROM locations ORDER BY name"))
        ).fetchall()
    await engine.dispose()

    assert count == 2
    assert rows == [("Bedfordview", "showroom"), ("Kramerville", "warehouse")]
