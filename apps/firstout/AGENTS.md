...
| Area | Sub-agent |
|------|-----------|
| `apps/firstout/backend` | `fastapi-backend` |
| `apps/firstout/frontend` | `frontend-dev` |
...
- Do **not** put Carbon in `libs/ui`. Do **not** run `add-ui-primitive` for Firstout.
...
- Auth cookie (S1, not S0): `firstout_session` — **not** `ht_session`.
- Import only: `f0rge_core`, `f0rge_db`, `f0rge_storage`, `f0rge_testing`. No Firstout domain in shared libs.
...
```bash
cd apps/firstout && docker compose up -d postgres
```

`DATABASE_URL=postgresql+asyncpg://firstout:firstout@localhost:5433/firstout`
...
```bash
cd apps/firstout && docker compose up -d postgres
cd apps/firstout/backend && uv run uvicorn app.main:app --port 8003 --reload
cd apps/firstout/frontend && npm run dev   # :3003, rewrites /api/* → :8003
```

### S1 auth bootstrap

On startup, if the `users` table is empty the API seeds one team (`Firstout`) and one owner from env:
...
Login requires `JWT_SECRET`. Cookie name is `firstout_session` (HttpOnly, SameSite=Lax, `Path=/`). Session cookie TTL is team-configurable via `team_settings.session_ttl_hours` (default **12 hours**, range 1–720); login sets JWT `exp` and cookie `Max-Age` from the current setting — no sliding refresh; changing the setting does not revoke already-issued tokens. Set `COOKIE_SECURE=true` when serving over HTTPS (Railway); leave `false` for local HTTP or the browser will not store the cookie.

Copy `apps/firstout/backend/.env.example` to `.env` and set a real `JWT_SECRET` before testing login locally. Generate `SETTINGS_ENCRYPTION_KEY` with Fernet (`cryptography.fernet.Fernet.generate_key()`) before saving an SMTP password in Settings → Communications.

### Communications (outbound SMTP)

Settings tab **Communications** (after Operations, before Nia). Owner (`settings.mutate`) saves one shop mailbox: host, port, STARTTLS/SSL/plain, username, password (Fernet-encrypted; GET never returns it), From, From name, Reply-To. `GET /api/v1/settings/comms` is any authenticated user (`smtp_configured`, `has_smtp_password`). `PATCH` and `POST /settings/comms/test-email` require `settings.mutate`. Test send uses stdlib `smtplib` in a worker thread. Missing mailbox → 503 `comms_smtp_unconfigured`. SMTP reject → 502 `comms_smtp_failed`. Missing/invalid `SETTINGS_ENCRYPTION_KEY` → 503 `comms_encryption_unconfigured`. Set the same Fernet key on Railway `firstout-api` develop. Nia still must not send.

### Permissions (F5)

Authorisation is a **permission catalog**, not role-tuple checks. `users.role` is a string slug matching `roles.slug`. Five built-in roles are **presets**. Owner is immutable (`is_system`, `is_owner_preset`); `has_permission` short-circuits for owner. Custom roles are named bundles (`POST /api/v1/roles`). Cookie remains `firstout_session`.
...
- **Stack:** PydanticAI + AG-UI on existing `firstout-api` (same cookie `firstout_session`). No CopilotKit, no extra Railway service, no Marrow `OPENROUTER_API_KEY`.
...
- **Env on `firstout-api` (develop):** `OPENROUTER_API_KEY` (required for LLM). Optional `OPENROUTER_MODEL` (default `deepseek/deepseek-v4-flash`; same on develop Railway). Optional `OPENROUTER_REASONING_EFFORT` (default `none`) + `OPENROUTER_REASONING_EXCLUDE` (default `false`) → pydantic-ai `openrouter_reasoning` / OpenRouter `reasoning`. `none` / `off` / empty+exclude false sends `enabled: false` (DeepSeek V4 Flash skips thinking). GLM-class models still reason; `exclude` only hides thinking from content. `OPENROUTER_BASE_URL` is stored on settings but **unused** — pydantic-ai 2.37 `OpenRouterProvider` has no `base_url`. Health: `GET /api/v1/nia/health` → `{ok, llm}` (`llm` false if key missing). Do **not** add these to `firstout-frontend`.
- **Railway click-path:** Railway → project **Firstout** → service **firstout-api** → environment **develop** → Variables → `OPENROUTER_API_KEY`. Never copy Marrow `zoological-fulfillment` keys into git or this project.
...
- **Threads (N1):** `GET/POST /api/v1/nia/threads`, `GET /api/v1/nia/threads/{id}`, `PATCH /api/v1/nia/threads/{id}` (rename), `POST /api/v1/nia/threads/{id}/archive` — require `nia.use`. List returns current user's non-archived threads only; cross-user GET/PATCH is 404. Auto-title from first user message when title is still `New thread`.
- **Run (N2):** `POST /api/v1/nia/threads/{id}/run` — require `nia.use`. Transport is **AG-UI SSE** via `pydantic_ai.ui.ag_ui.AGUIAdapter` (`streaming_response` + `run_stream`). Accepts either canonical AG-UI `RunAgentInput` JSON (`threadId`, `runId`, `messages`, `tools`, `context`, `forwardedProps`) or convenience JSON `{ "message": "…", "page": { "path": "/invoices" } }` (thread id from path; server generates `runId`). On completion: append user + assistant rows to the thread, persist the PydanticAI `all_messages()` history in `nia_threads.agent_messages` (20 most recent user turns) for the next run's `message_history`, and insert a `nia_usage_events` row (`input_tokens`/`output_tokens` → prompt/completion). Missing `OPENROUTER_API_KEY` → **503** `{"detail":{"code":"nia_llm_unconfigured"}}` (never echo the key). `check_nia_budget` runs before OpenRouter (N5). CI/tests: `models.ALLOW_MODEL_REQUESTS = False` + `TestModel` override + dummy key.
...
- **Tools (N4):** `app/nia/tools.py` — in-process wrappers around existing services (not HTTP self-calls). `NiaDeps` carries `db: AsyncSession` + permission keys. Special tools: `navigate` (allowed paths from frontend `nav.ts` + `/canvas`), `search` (`SearchService`), `list_overdue_invoices` (30-day CRM clock), `get_stock_on_hand` (`InventoryService`; omits `unit_cost_zar` without `stock.cost.view`), `propose_transfer` (`TransferService.create` draft only, name-based), Canvas tools (`clear_canvas`, `set_canvas`, `add_canvas_component`, `remove_canvas_component`, `set_canvas_title`, `chart_dining_vs_sofas`, `chart_overdue_invoices`, `chart_sales_by_sku`, `chart_stock_on_hand`, `chart_aged_ar` — numbers from existing report/stock/invoice services). **Catalog (T0 / #596):** `app/nia/catalog.py` + `list_nia_actions` / `run_nia_action` in `dispatch.py`. One `NiaAction` registry — reads + writes the login may run (`permission` is `None` or `has_any`). Every write raises `ApprovalRequired` (`needs_ok`) before mutate; permission miss is a tool **string** naming the key (e.g. `catalogue.mutate`). Never catalog: till sale / card capture, auth login/logout/me, Nia threads/run/resume/archive/usage/caps, health, email, SARS eFiling, file uploads (SKU photo, bank CSV, bill attachment, catalogue/journal CSV, proforma/land PDFs are catalog ids but return an upload hint — no files). Keep the special tools above. Catalog also has `create_transfer` (`TransferCreate` UUIDs) beside name-based `propose_transfer`. **Till cannot transfer** — `propose_transfer` returns a denial string (no HITL). Users with `stock.transfer` raise `ApprovalRequired` before create. `demo_echo_approval` unchanged. Tests: `tests/test_nia_tools_rbac.py`, `tests/test_nia_catalog.py` — `TestModel` / `ArgsTestModel` tool forcing.
- **Caps (N5):** Alembic `039_nia_caps` — `team_settings.nia_monthly_token_cap` (default **500000**), nullable `users.nia_monthly_token_cap` override (`null` = inherit team default). Effective cap = user override when set, else team default. **`0` = blocked** (no runs). Usage = sum of `nia_usage_events.total_tokens` for `user_id` in the **current UTC calendar month** (N2 inserts usage after the call — cap check does not double-count). At/over cap → **429** `{"detail":{"code":"nia_cap_exceeded"}}` before OpenRouter; used unchanged. Endpoints: `GET /api/v1/nia/usage/me` (`nia.use`), `GET /api/v1/nia/usage` + `PATCH /api/v1/nia/usage/{user_id}` (`nia.admin`; till → 403 on others). Team default also on `GET/PATCH /api/v1/settings` (`settings.mutate`). Tests: `tests/test_nia_caps.py`.
- **Dock (N6):** Right-side Carbon panel in `app-shell.tsx` for users with `nia.use` (`canUseNia`). **Cmd/Ctrl+.** or header Nia mark toggles open/closed. Drag left edge to resize (sessionStorage); Expand opens a Carbon Modal overlay (dock column width unchanged). Microphone control uses Carbon `Microphone` icon with `aria-label="Dictate"` (Web Speech API; append transcript, no auto-send). Product name **Nia** only — never Copilot. `POST …/run` and accept/decline `…/resume` are AG-UI SSE — **consume the full stream** then `GET` thread; cancel resume is JSON. Structured cards for `needs_ok` / `needs_fields` / `your_call` / `overdue_invoices` / `canvas_spec` / `canvas_cleared` / `transfer_draft` — never raw JSON in chat. `canvas_spec` dock cards are citation/nav only (title + **View Canvas**); do **not** embed Jspreadsheet in the dock — tables live on `/canvas`. Omit View Canvas on `canvas_spec` / `canvas_cleared` cards when the dock is already on `/canvas`. Small lists stay Carbon/markdown (`overdue_invoices`); rich tabular BI uses canvas tools. `opened_page` still navigates via the dock; do not also render a View-page button for that same turn. Assistant bubbles render markdown (no raw HTML). While a run is in flight, show “Nia is working…” plus a collapsed Thinking accordion (reasoning / tool names). History via header control (not a permanent thread list). `PATCH /nia/threads/{id}` renames threads. Write tools with missing required fields emit `needs_fields`; submit via `POST …/resume` `{ "decision": "submit_fields", "fields": {…} }`, then the usual `needs_ok` before mutate.
- **Resume + undo (N8):** `transfer_draft` cards refresh via `GET /transfers/{id}`; Undo on draft calls `POST /transfers/{id}/cancel` (409 → hide Undo, “Already dispatched — reverse in Transfers”). Citation chips on structured payloads; never expose OpenRouter keys.
- **Canvas (N7):** Canvas tools emit `canvas_spec` (allowlisted `bar` | `line` | `table` | `metric` — never HTML) or `canvas_cleared`. Spreadsheet is a **frontend renderer** for existing `table` components (`headers: string[]`, `rows: string[][]`) via Jspreadsheet CE on `/canvas` only (Carbon-themed, read-only) — do not invent an HTML canvas type; never mount it in the Nia dock. Frontend `/canvas` (SideNav for `nia.use`) renders the spec; latest in `localStorage` key `firstout-nia-canvas-state` (user-scoped). `clear_canvas` and the page Clear button wipe the store so hydrate does not resurrect an older thread chart. Tests: `tests/test_nia_canvas.py`, `tests/test_nia_canvas_tools.py`.
- **Audit (N8):** Alembic `040_nia_audit` — append-only `nia_audit_events` for HITL accept/decline/cancel (not journals). Citations on structured payloads (`transfer_draft`, `overdue_invoices`). Undo = existing `POST /transfers/{id}/cancel` on draft only. Resume persists `deps.last_structured_payload` on assistant message. `GET /api/v1/nia/threads/{id}/audit` (owner-only 404; redacts cost keys in args without `stock.cost.view`). Tests: `tests/test_nia_audit.py`.
...
**Railway develop:** on service `firstout-api`, set `SEED_PLAYGROUND=true` and redeploy (or restart). Safe to leave on — second boot is a no-op. Do **not** enable on production. After it runs, log in as `owner@example.com` / `change-me-owner` (or the role users above) and walk stock → proforma → PO → receive → transfer → till → books.

Local: `SEED_PLAYGROUND=true` in `apps/firstout/backend/.env`, then restart uvicorn.
...
Endpoints (cookie `firstout_session`): `GET/POST /api/v1/transfers`, `GET /transfers/{id}`, `POST /transfers/{id}/dispatch`, `GET /transfers/{id}/pdf`, `POST /transfers/{id}/receive`, `POST /transfers/{id}/cancel`.
...
Endpoints (all under `/api/v1`, cookie `firstout_session`):
...
Endpoints (cookie `firstout_session`): `POST /api/v1/imports/preview`, `POST /api/v1/imports/commit`. Multipart: `inventory` CSV required, `soh` CSV optional, optional JSON strings `inventory_map` / `soh_map`. No GET. **`catalogue.mutate`** (`require_catalogue_mutate`). Warehouse-only SOH import is deferred.
...
Endpoints (cookie `firstout_session`):
...
Richer stock and sales reports under `/api/v1/reports` (cookie `firstout_session`). JSON + CSV export for each. All authenticated roles (same as existing S7 reports).
...
`GET /home` (cookie `firstout_session`) extends the S10 summary with hub KPIs and attention lists. Existing fields (`on_order_*`, `on_hand_*`, `home_currency`) unchanged.
...
**Own Railway project** — not Marrow `zoological-fulfillment`, not the Marrow develop environment, not Marrow Postgres/Redis/photos. Do not add `firstout-*` services to the Marrow project. Production has no Firstout services; the Marrow project has no Firstout config.

| Resource | ID / URL |
|----------|----------|
| Project `Firstout` | `c76d8df1-d839-454c-a94a-79b930deaf38` |
| Environment `develop` | `7a8ce6f1-c514-4e5d-9c50-4b7918865321` |
| Service `firstout-api` | `77a83033-b75e-4da1-88d0-b6db39bcedaf` |
| Service `firstout-frontend` | `10838be1-7a37-4dfc-810a-8805d040d5d7` |
| Postgres (own, not Marrow pgvector) | `a1a6695d-b8fb-4c79-92a6-664b48bc07e4` |
| Bucket `firstout-dev` (Tigris, develop only) | `49435225-4849-4132-bb87-66a23c67cdf1` |
| API | https://api.firstout.co.za (`/api/v1/health`, Swagger `/docs`) |
| Frontend | https://firstout.co.za |

- **Replicas:** 1 each (hobby tier, `sfo` region).
- **Config files:** `apps/firstout/{backend,frontend}/railway.toml` — no Root Directory; Config File path points here.
- **`watchPatterns`:** `apps/firstout/**` + `libs/backend/{core,db,storage}/**` (repo `railway.toml` and live `firstout-api`). Dockerfile `COPY`s `libs/backend/storage` for `f0rge_storage`.
- **Manifest:** `.github/deploy/manifest.yml` — `branches: [develop]` only. No `health_url.main`, no production.
- **Auth bootstrap:** on first deploy with empty `users`, seeds owner from `SEED_OWNER_EMAIL` / `SEED_OWNER_PASSWORD` (defaults `owner@example.com` / `change-me-owner`). Every startup also seeds missing role users `till@` / `books@` / `warehouse@` / `buyer@example.com` (`SEED_*_PASSWORD`, defaults `change-me-<role>`). Cookie `firstout_session` (HttpOnly, SameSite=Lax, Secure on HTTPS).
- **Comms encryption:** `SETTINGS_ENCRYPTION_KEY` (Fernet) on `firstout-api` develop — required to save SMTP/WhatsApp secrets. Same value locally in `.env`.
...
- **Playground dataset:** set `SEED_PLAYGROUND=true` on `firstout-api` and redeploy to fill catalogue / PO / till / books for demos. Default off. See [Playground seed](#playground-seed-develop--local-demos).
- **Object storage:** dedicated Railway Tigris bucket `firstout-dev` in this project only — never Marrow `photos` / `photos-dev`, never Marrow project buckets. On `firstout-api` develop: `BUCKET_NAME` / `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` reference `${{firstout-dev.*}}`; `AWS_ENDPOINT_URL_S3=https://fly.storage.tigris.dev`; `AWS_REGION=auto`. Keep `COOKIE_SECURE`, `JWT_SECRET`, `DATABASE_URL`. When those AWS vars are unset (local), uploads use `STORAGE_DIR`. Production is not wired.

## V2 shell (issue #530)

Superdesign canvas (try-first; record credits failure in PR if CLI blocks): [Firstout Back Office Home v2](https://superdesign.dev/teams/cb0bbbcd-2f7f-4810-9426-2fbdd5577264/projects/21ee8b12-d1ca-40c6-9b91-312aeb11a9f7) — draft `2d67462c-32c2-4baf-90c8-bdef3893e5bd`. Saved copy: `apps/firstout/frontend/.superdesign/v2-home.html`.

- **UI:** IBM Carbon only — never `@f0rge/ui`, Tailwind, shadcn, or Mantine in this app.
- **Chrome:** Carbon UIShell; content `g10`; SideNav dark via `Theme g100`; main offset `.firstout-main` **16rem expanded / 3rem collapsed** (`data-nav-expanded`).
...
`uv --project apps/firstout/backend`. **Never** create a root `uv.lock`. Python 3.10 only (`from __future__ import annotations`; no `X | Y`).
