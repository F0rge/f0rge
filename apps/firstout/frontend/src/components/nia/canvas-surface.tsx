...
function BarChart({ component }: { component: CanvasBarLineComponent }) {
  ...
  return (
    <div className="firstout-canvas-chart">
      <p className="cds--type-heading-compact-01">{component.title}</p>
      <div className="firstout-canvas-chart__legend">
        {series.map((entry, index) => (
          <span key={entry.name} className="firstout-canvas-chart__legend-item">
            <span
              className="firstout-canvas-chart__swatch"
      ...
      <div className="firstout-canvas-chart__bars">
        {component.categories.map((category, categoryIndex) => (
          <div key={`${component.id}-${category}`} className="firstout-canvas-chart__group">
            <p className="firstout-canvas-chart__category">{category}</p>
            {series.map((entry, seriesIndex) => {
              const value = entry.values[categoryIndex] ?? 0;
              const widthPct = Math.max(0, (value / maxValue) * 100);
              return (
                <div key={`${entry.name}-${category}`} className="firstout-canvas-chart__bar-row">
                  <span className="firstout-canvas-chart__bar-label">{entry.name}</span>
                  <div className="firstout-canvas-chart__bar-track">
                    <div
                      className="firstout-canvas-chart__bar-fill"
                      style={{
                        width: `${widthPct}%`,
                  ...
                  <span className="firstout-canvas-chart__bar-value">{formatChartNumber(value)}</span>
                </div>
              );
            })}
          </div>
...
function LineChart({ component }: { component: CanvasBarLineComponent }) {
  ...
  return (
    <div className="firstout-canvas-chart">
      ...
      <svg
        className="firstout-canvas-chart__line-svg"
      ...
      <div className="firstout-canvas-chart__axis">
        {component.categories.map((category) => (
          <span key={category} className="firstout-canvas-chart__axis-label">
            {category}
          </span>
        ))}
      </div>
...
function MetricTile({ component }: { component: Extract<CanvasComponent, { type: "metric" }> }) {
  return (
    <Tile className="firstout-canvas-metric">
      <p className="cds--type-label-01">{component.label}</p>
      <p className="firstout-canvas-metric__value">{component.value}</p>
    </Tile>
  );
}
...
export function CanvasSurface({ spec }: CanvasSurfaceProps) {
  return (
    <div className="firstout-canvas-surface">
      {spec.components.map((component) => (
        <section key={component.id} className="firstout-canvas-surface__block">
