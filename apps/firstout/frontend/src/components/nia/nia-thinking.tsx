...
export function NiaMilestoneList({ labels }: { labels: string[] }) {
  ...
  return (
    <ul className="firstout-nia-dock__milestones">
      {lines.map((label, index) => (
        <li key={`${index}-${label}`} className="firstout-nia-dock__milestone">
          {label}
        </li>
      ))}
    </ul>
...
export function NiaThinking({
  ...
  return (
    <div className="firstout-nia-dock__thinking">
      {working ? (
        <div className="firstout-nia-dock__working">
      ...
      {showReasoning ? (
        <Accordion align="start" size="sm">
          ...
          >
            {body ? <pre className="firstout-nia-dock__thinking-body">{body}</pre> : null}
          </AccordionItem>
        </Accordion>
      ) : null}
    </div>
