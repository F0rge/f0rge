...
import { useExtractTreatmentUpload } from '@/lib/api/hooks'
...
export function TreatmentUploadDialog({ open, onOpenChange }: TreatmentUploadDialogProps) {
  const [phase, setPhase] = useState<Phase>('pick')
  const [dragOver, setDragOver] = useState(false)
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [result, setResult] = useState<TreatmentExtractionResult | null>(null)
  const [confirmOpen, setConfirmOpen] = useState(false)
  const fileRef = useRef<HTMLInputElement>(null)

  const extractUpload = useExtractTreatmentUpload()
    ...
    <>
      <Dialog open={open && !confirmOpen} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a PDF or image and AI will extract medications for you.
            </DialogDescription>
          </DialogHeader>
          ...
          {phase === 'extracting' && (
            <div className="flex flex-col items-center gap-4 py-10">
              ...
              <div className="text-center">
                <p className="text-sm font-medium">Extracting prescription data...</p>
                {selectedFile && (
          ...
          {phase === 'review' && result && (
            <div className="space-y-4">
              <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                Extracted {result.payload.treatments.length} treatment
                {result.payload.treatments.length === 1 ? '' : 's'} &middot; confidence{' '}
      ...
      {result && (
        <TreatmentReviewDialog
          key={result.raw_response}
          open={confirmOpen}
          onOpenChange={(o) => {
            setConfirmOpen(o)
            if (!o) handleClose(false)
          }}
          candidates={result.payload.treatments}
          extractionMeta={{
            model: result.model,
