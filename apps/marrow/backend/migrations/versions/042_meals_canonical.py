...
def upgrade() -> None:
    ...
    # Drop duplicate copy-local analyses; keep the row on the canonical (non-copy) photo.
    op.execute(
        sa.text(
            """
...
def downgrade() -> None:
    op.execute(sa.text("DROP POLICY IF EXISTS photo_ingredients_meal_participant_update ON photo_ingredients"))
    op.execute(sa.text("DROP POLICY IF EXISTS photo_ingredients_meal_participant_select ON photo_ingredients"))
    op.execute(sa.text("DROP POLICY IF EXISTS photo_analyses_meal_participant_update ON photo_analyses"))
    op.execute(sa.text("DROP POLICY IF EXISTS photo_analyses_meal_participant_select ON photo_analyses"))
