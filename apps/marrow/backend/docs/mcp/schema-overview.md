...
Write operations are not available through MCP except `update_hypothesis` and
`update_n_of_1`, which use the app role (`scoped_main_session`), never
`healthtracker_ro`.
...
| Hypotheses | `hypotheses`, `n_of_1_slots` | Ranked questions, kill-tests, one n-of-1 slot |
...
| `marrow://schema/hypotheses` | Before listing or updating the hypothesis scoreboard |
