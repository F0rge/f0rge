...
async def test_update_llm_model(async_db: AsyncSession) -> None:
    svc = SettingsService(async_db)
    resp = await svc.update_llm(LLMSettingsUpdate(llm_model="anthropic/claude-haiku-4-5"))
    assert resp.llm_model == "anthropic/claude-haiku-4-5"


async def test_update_embedding_does_not_accept_a_key(async_db: AsyncSession) -> None:
    """There is only one stored BYOK key (set via update_llm). update_embedding has no
    key field at all — setting the model/provider must not touch has_api_key."""
    svc = SettingsService(async_db)
    await svc.update_llm(LLMSettingsUpdate(llm_api_key="shared-secret"))
    resp = await svc.update_embedding(EmbeddingSettingsUpdate(embedding_model="m/embed"))
    assert resp.has_api_key is True
    assert resp.embedding_model == "m/embed"
...
async def test_update_is_idempotent_upsert(async_db: AsyncSession) -> None:
    ...
    count = (await async_db.execute(select(func.count()).select_from(UserSettings))).scalar_one()
    assert count == 1
