...
async def serialize_entry(db: AsyncSession, source_id: int) -> Optional[str]:
    ...
    # Effective flags: combine photo-derived flags + user-added assertions for accuracy
    _user_added = _parse_diet_risk_csv(row.diet_risk)
    _photo_flags = compute_photo_signal(row).flags
    _effective = sorted(_photo_flags | _user_added)
