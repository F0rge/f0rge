...
async def provision_user_catalogs(db: AsyncSession, user_id: uuid.UUID) -> None:
    ...
    await _insert_trackers(db, user_id)
    await _copy_reference_catalogs(db, user_id)
...
async def _insert_trackers(db: AsyncSession, user_id: uuid.UUID) -> None:
    values = [
        {
            "user_id": user_id,
            "name": name,
    ...
    stmt = (
        insert(Tracker).values(values).on_conflict_do_nothing(constraint="uq_tracker_user_id_name")
    )
    await db.execute(stmt)


async def _copy_reference_catalogs(db: AsyncSession, user_id: uuid.UUID) -> None:
    ...
    await db.execute(
        ...
        {"new_user_id": str(user_id), "ref_user_id": str(ref_user_id)},
    )
