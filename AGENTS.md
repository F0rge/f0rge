...
- `develop` is the integration branch; PRs land there, gated by `.github/workflows/ci.yml`, then merge.
- Promotion to prod is a PR `develop` → `main`, gated by the same `ci.yml` (prod-shaped frontend build on `main`).
