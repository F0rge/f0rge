...
- `.github/workflows/**`
- `docker-compose*.yml` (currently `docker-compose.dev.yml`, `docker-compose.prod.yml`, and the split-out MCP apps `docker-compose.mcp.dev.yml`, `docker-compose.mcp.prod.yml`)
...
- **`mem_limit:` legacy v2 style only.** All compose files use `mem_limit:` consistently (`docker-compose.prod.yml`: postgres 768m, postgres-backup 128m, backend 512m, embedding-worker 512m; `docker-compose.mcp.prod.yml`: mcp-server 256m). Block any new service that uses `deploy.resources.limits.memory:` instead. Match the existing style block. Cite: `project_mcp_phase_2_2c_infra.md`.
